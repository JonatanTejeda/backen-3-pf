...
export const create = async (req, res, next) => {
  try {
    ...
    if (!newCart) res.status(404).json({ msg: "¡Error al crear carrito!" });
...
export const remove = async (req, res, next) => {
  try {
    ...
    if (!cartDel) res.status(404).json({ msg: "¡Error en el carrito de actualización!" });
...
export const addProdToCart = async (req, res, next) => {
  try {
    ...
    if (!newProdToUserCart) res.json({ msg: "Error al añadir un producto al carrito" });
...
export const removeProdToCart = async (req, res, next) => {
  try {
    ...
    if (!delProdToUserCart) res.json({ msg: "Error eliminar el producto al carrito" });
...
export const updateProdQuantityToCart = async (req, res, next) => {
  try {
    ...
    if (!updateProdQuantity) res.json({ msg: "Error al actualizar la cantidad del producto al carrito" });
...
export const clearCart = async (req, res, next) => {
  try {
    ...
    if (!clearCart) res.json({ msg: "Error al borrar el carrito" });
